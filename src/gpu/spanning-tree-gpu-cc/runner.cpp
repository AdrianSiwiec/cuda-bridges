...
void load_graph(edge** ed_list,int*num_n,int*num_e, char* argv[]){
    FILE*fp,*fp2;
    edge*edl;
    int i,j,x,y,a;
    long long int v;
...
{
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " PARAMS EDGES" << std::endl;
        exit(1);
    }
...
//    checkCudaErrors( cutStartTimer( timer1));
    ...
    load_graph(&ed_list,&num_n,&num_e, argv);
    ...
    // MY CODE FOR HOOK EDGES
    int * d_hook_edge;
    int * hook_edge = (int*)calloc(num_n,sizeof(int));
    
    checkCudaErrors(cudaMalloc((void**)&d_hook_edge, num_n*sizeof(int)));
...
//     select_winner_init<<< grid_e,threads>>>(d_an,d_ed_list,num_e,num_n,d_flag,d_mark, d_hook_edge);
//     cudaThreadSynchronize();

//     select_tree_edges_and_merge_init<<< grid_n,threads>>>(d_an,d_ed_list,num_e,num_n,d_flag,d_mark, d_hook_edge);
//     cudaThreadSynchronize();
...
//     }while(flag);
    ...
    do{
        ...
        checkCudaErrors(cudaMemcpy(d_flag,&flag,sizeof(int),cudaMemcpyHostToDevice));
        if(lpc!=0){
            select_winner<<< grid_e,threads>>>(d_an,d_ed_list,num_e,num_n,d_flag,d_mark, d_hook_edge);

            cudaThreadSynchronize();
            select_tree_edges_and_merge<<< grid_n,threads>>>(d_an,d_ed_list,num_e,num_n,d_flag,d_mark, d_hook_edge);

            lpc++;
            lpc=lpc%4;
        }
        else{
            select_winner2<<< grid_e,threads>>>(d_an,d_ed_list,num_e,num_n,d_flag,d_mark, d_hook_edge);

            cudaThreadSynchronize();
            select_tree_edges_and_merge2<<< grid_n,threads>>>(d_an,d_ed_list,num_e,num_n,d_flag,d_mark, d_hook_edge);
...
//        CUT_CHECK_ERROR("Kernel execution failed");
    ...
    checkCudaErrors(cudaMemcpy(ed_list,d_ed_list,num_e*sizeof(edge),cudaMemcpyDeviceToHost));

    std::ofstream ofs_params, ofs_edges;
    ofs_params.open("../../../3rdparty/GpuConnectedComponents/parameters.txt", std::ofstream::out | std::ofstream::trunc);
    ofs_edges.open("../../../3rdparty/GpuConnectedComponents/edge.txt", std::ofstream::out | std::ofstream::trunc);
